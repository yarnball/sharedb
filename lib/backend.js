...
Backend.prototype.addProjection = function(name, collection, fields) {
  let res = { [name]:{target: collection } }
  ...
  if(fields){
    ...
    res = { [name]:{target: collection, fields }}  
  }
  ...
  backend.trigger(backend.MIDDLEWARE_ACTIONS.submit, agent, request, function(err) {
    ...
    request.submit(function(err) {
      ...
      backend.trigger(backend.MIDDLEWARE_ACTIONS.afterSubmit, agent, request, function(err) {
        if (err) return callback(err);
        var projection = request.projection && request.projection.fields && request.projection
        var collection = request.projection && request.projection.target || request.collection
        backend._sanitizeOps(agent, projection, collection, id, request.ops, function(err) {
          if (err) return callback(err);
...
Backend.prototype.getOps = function(agent, index, id, from, to, callback) {
  var start = Date.now();
    var projection = this.projections[index].fields && this.projections[index];
  // ONLY define projection IF fields exist
  var collection = (this.projections[index].target) ? this.projections[index].target : index
  var backend = this;
  var request = {
    agent: agent,
    index: index,
